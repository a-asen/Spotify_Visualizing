...
# example: 
# my_recently_played = user_last_played()
...
# Example:
# track_analysis = track_analysis_to_df(<my_top_tracls["uri"]>)

# %%  Read  //  Get data
if os.path.exists("data/my_top_50.csv"): # check if we already have data 
    # no need to load the api unecessarily
    my_top_tracks = pd.read_csv("data/my_top_50.csv")
else: # if not, we get the data and save it
    my_top_tracks = user_top_tracks_df() # get my top 50 songs (so far: 2023.03.11)
    my_top_tracks.to_csv("data/my_top_50.csv", index=False) # save it 
    
if os.path.exists("data/my_top_50_analysis.csv"):
    my_top_track_analysis = pd.read_csv("data/my_top_analysis.csv")
else:
    my_top_tracks_analysis = track_analysis_to_df(my_top_tracks["uri"]) # get features
    my_top_tracks_analysis.to_csv("data/my_top_50_analysis.csv",index=False)
    
# %% playlist analysis
"""
make culminative time ! so we can see whe nin the song we are lookin at 
...
my_top_song_features = my_top_tracks_analysis[my_top_tracks_analysis["song index"] == 0] # subset of my top song.

#%%
sb.displot(my_top_song_features,x = "track index", 
                y = "timbre-3")

#%%
sb.relplot(my_top_song_features, x = "track index", 
           y = "timbre-5",
           kind="line")
